...
def port_detection(ip):
    ...
    # list_ports = [int(port) for port in list_ports]
    # list_ports = list_ports[:100]
...
def nmap_scan():
    nm = nmap.PortScanner()
    result = nm.scan(hosts="192.168.102.0/24", arguments="-sn")
    print(result)
    connected_ip = []
...
def scan_ip():
    arp_request = ARP(pdst="192.168.102.0/24")
